 ...
 */
@Service
public class DateDownload4SsServiceImpl extends Hz2004BaseService implements
        DateDownload4SsService {

    //日志处理
    protected static Log _log = LogFactory.getLog(DateDownload4SsServiceImpl.class);

    public DateDownload4SsServiceImpl() {
    }
...
//                        this.log(logPath, "XT_XZQHB", Cxdctj,
//                                 "成功新建文件,正在进行数据导入:" +
...
//            this.log(logPath, "XT_XZQHB", Cxdctj, "全部数据导出完毕,所有连接正确释放.");

            //this.ftp(Yhid, DcwjlbList);

            log_update("2", "数据导出成功,请及时下载", YhidTwo);

//            this.log(logPath, "XT_XZQHB", Cxdctj, "全部文件上传FTP成功,服务器端数据文件回滚.");
...
//            this.log(logPath, "XT_XZQHB", Cxdctj, "数据导出异常,全部数据文件回滚.");
...
//            this.log(logPath, "XT_XZQHB", Cxdctj, "全部数据导出完毕,权限正确释放.");
