Gnt.ux.SelectGghWhModify= Ext.extend(Ext.Window, {
	title:'公共户标识修改',
	closeAction:'hide',
	modal:true,
	width:300,
	...
	show:function(selRes,type){
		...
		Gnt.ux.SelectGghWhModify.superclass.show.call(this);
	},
		...
		if(type=='single'){
			if(selRes){
				this.fs.getForm().setValues({
					...
					jttdbz: selRes.data.jttdbz=='1'?selRes.data.jttdbz:''
				});
			}
	    	...
	    	buttons:[
	    	 		{
	    	 			...
	    	 			minWidth:75,
	    	 			handler:function(){
	    	 				var rootWin = this.findParentByType("modifyGghwh_window");
...
//	    	 				}
	    	 			...
	    	 			minWidth:75,
	    	 			handler:function(){
	    	 				var win = this.findParentByType("modifyGghwh_window");
	    	 				win.hide();
	    	 			}
	    	 		}
	    	 	]
		...
		Gnt.ux.SelectGghWhModify.superclass.initComponent.call(this);
	}
	
	
});
Ext.reg('modifyGghwh_window', Gnt.ux.SelectGghWhModify);
