...
<%
	...
	String pcsfw = "";
	String xzqhfw = "";
	List<PoXT_YHSJFWB> l= u.getSjfw();
	for(int i = 0 ;i<l.size();i++){
		if(l.get(i).getXqlx().equals("1")){
			String sjfwstr = l.get(i).getSjfw();
			String sjfw[] = sjfwstr.split("\\|");
			for(String bm:sjfw){
				bm = bm.trim();
				
				if(CommonUtil.isEmpty(bm))
					continue;
				
				if(sjfw.length==1&&bm.length()==6)
					xzqhfw += bm+",";
				else if(sjfw.length==2&&bm.length()==9)
					pcsfw += bm + ",";
			}
		}
	}
...
<body>
	...
	<script type="text/javascript" src="static/cx/hjjbxx/queryRkxx.js?v=<%=Math.random()%>"></script>
	...
	<script type="text/javascript" src="js/pointer_events_polyfill.js?v=<%=Math.random()%>"></script>
	<script type="text/javascript" src="js/watermark.js?v=<%=Math.random()%>"></script> 
