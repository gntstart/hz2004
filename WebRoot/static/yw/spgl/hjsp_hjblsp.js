...
Ext.onReady(function(){
	...
	var spxx_grid = new Gnt.ux.SjpzGrid({
		...
		region:'center',
    	title: '查询结果',
    	url: 'yw/spgl/hjsp/queryHjblspxxByUser.json',
		listeners:{
		         ...
		         new Ext.Button({
						...
						handler:function(){
							...
							(function(){
								...
								spcl_grid.loadData('5', selRes.data.spywid);
							}).defer(200);
						}
    	         }),
    	         new Ext.Button({
							...
							handler:function(){
								...
								Gnt.makeHzyw({
		   						...
		   						handler:function(){
		   							...
		   							var list = [];
		   							spzxx_grid2.store.each(function(r){
		   								var sub_data = ywForm.getForm().getValues();
		   								sub_data.dzid = r.data.xydzid;
		   								sub_data.spywid = r.data.spywid;
		   							...
		   							Gnt.submit(
		   									{
		   										voHjblspspxx: Ext.encode(list) //Ext.encode([sub_data])
		   									}, 
		   									"yw/spgl/hjsp/processHjblspspyw", 
		   									"正在处理审批信息，请稍后...", 
		...
		items:[
				{
		    	   ...
		    	   frame:false,
				   borde:false,
		    	   html: '<center><br/><br/><br/><br/><h2>户籍补录审批业务完成！</center></h2>',
			   		buttons:[
			 	           new Ext.Button({
								text:'新办理',
								minWidth:100,
