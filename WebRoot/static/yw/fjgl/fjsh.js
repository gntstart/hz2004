...
//		height:300,
	...
	shGgDialog = new Gnt.ux.SelectShGgDialog({
		callback: function(optype, bhgData){
			...
			var ksh = bhgData.ksh;
			var jsh = bhgData.jsh;
			for(var i=ksh-1;i<fjshGrid.store.totalLength&&i<jsh;i++){
				var data = {};
				var griddata = fjshGrid.store.getAt(i).data;
...
//					}
						        			...
						        			handler:function(){
						        				...
						        				store.on('load',function(s,records){
						        					...
						        					for( var i  = 0;i< store.data.length;i++){
						        						var selResTemp = fjshGrid.store.getAt(i);
			    		    ...
			    		    items:[new Ext.Button({
			        			...
			        			handler:function(){
			        				...
			        				if(parent && parent.openWorkSpace){
			        					parent.openWorkSpace(url,  "分局审核对照", "fjshdz" );
			        				}else{
	...
	function applyParam(store){
		...
		Ext.apply(store.baseParams, {yshxx:yshxx,kzxx:kzxx,yzw:yzw});
