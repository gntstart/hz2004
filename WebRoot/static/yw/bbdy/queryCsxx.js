...
Ext.onReady(function(){
	...
	//本业务需要加载的配置
	if(!Gnt.loadSjpzb("20006",function(){}))
		return;
	//本业务需要加载的字典
...
//				sbdy_window.show();
//				sbdy_window.setSelRes(data,"cs",zdKeyArray,zdValueArray);
				var param = rkxxGrid.store.baseParams;
				if(pxzdValue){
					param.pxzdValue = pxzdValue
				}
				param.config_key='querySbcsxx';
				Gnt.submit(
						param, 
						...
						function(jsonData,params){
							...
							sbdy_window.show();
							sbdy_window.setSelRes(dcdata,"cs",zdKeyArray,zdValueArray,dictMap);
						}
				);
			}else{
				alert("设置字段方法出错了！")
		...
		buttons:[
			{
				...
				minWidth:60,
				handler:function(){
					urlLink = "yw/common/downExcelZip?type=queryCsxx";
...
//								var dcdata = jsonData.list;
//								sbdy_window.show();
//								sbdy_window.setSelRes(dcdata,"cs",null,null,dictMap);
//							}
//					);
				}
			},{
	...
	store.baseParams.daochuFlag = 'queryCsxx';
	store.load();
	Ext.Msg.hide();
