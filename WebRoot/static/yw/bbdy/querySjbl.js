...
Ext.onReady(function(){
	...
	//本业务需要加载的配置
	if(!Gnt.loadSjpzb("20010",function(){}))
		return;
	//本业务需要加载的字典
...
//				sbdy_window.show();
//				sbdy_window.setSelRes(data,"sjbl",zdKeyArray,zdValueArray);
				var param = rkxxGrid.store.baseParams;
				if(pxzdValue){
					param.pxzdValue = pxzdValue
				}
				param.config_key='querySbsjbl';
				Gnt.submit(
						param, 
						...
						function(jsonData,params){
							...
							sbdy_window.show();
							sbdy_window.setSelRes(dcdata,"sjbl",zdKeyArray,zdValueArray,dictMap);
						}
				);
			}else{
				alert("设置字段方法出错了！")
...
//							var dcdata = jsonData.list;
//							sbdy_window.show();
//							sbdy_window.setSelRes(dcdata,"sjbl",null,null,dictMap);
//						}
//					);
				}
			},{
	...
	store.baseParams.daochuFlag = 'querySjblxx';
	store.load();
	Ext.Msg.hide();
