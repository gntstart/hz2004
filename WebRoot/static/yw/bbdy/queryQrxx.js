...
Ext.onReady(function(){
	...
	//本业务需要加载的配置
	if(!Gnt.loadSjpzb("20003",function(){}))
		return;
	//本业务需要加载的字典
...
//				sbdy_window.show();
//				sbdy_window.setSelRes(data,"qr",zdKeyArray,zdValueArray);
				var param = rkxxGrid.store.baseParams;
				if(pxzdValue){
					param.pxzdValue = pxzdValue
				}
				param.config_key='querySbqrxx';
				Gnt.submit(
						param, 
						...
						function(jsonData,params){
							...
							sbdy_window.show();
							sbdy_window.setSelRes(dcdata,"qr",zdKeyArray,zdValueArray,dictMap);
						}
				);
			}else{
				alert("设置字段方法出错了！")
...
//							var dcdata = jsonData.list;
//							sbdy_window.show();
//							sbdy_window.setSelRes(dcdata,"qr",null,null,dictMap);
//						}
//					);
				}
			},{
	...
	store.baseParams.daochuFlag = 'queryQrxx';
	store.load();
	Ext.Msg.hide();
