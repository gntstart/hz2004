...
var cxCount = 1;
var rynbid = null;
var htmlStrStart = '<iframe scrolling="auto" frameborder="0" width="100%" height="100%" src="cx/pzzpxx/getPzxx';
...
Ext.onReady(function(){
	...
	var form20035 = new Gnt.ux.SjpzForm({
		pzlb: '20035',
		url: 'cx/pzzpxx/getPzxx',
		title:'',
		cols:2,
		formType:'query'
	});
	var grid20001 = new Gnt.ux.SjpzGrid({
		title: '迁入人员列表',
		region : 'south',
		height:180,
	...
	var pzlogGrid = new Gnt.ux.SjpzGrid({
		pkname: 'pzrzid',
		pzlb: '20035',
		title: '',
		url: 'cx/pzzpxx/getPzxx',
	...
	var pzxxGrid = new Gnt.ux.SjpzGrid({
		pkname: 'pzrzid',
		pzlb: '20035',
		title: '',
		url: 'cx/pzzpxx/delPzxx',
		showPaging:false
	});
	var NewpzlogGrid = new Gnt.ux.SjpzGrid({
		pkname: 'pzrzid',
		//region:'north',
		pzlb: '20035',
		title: '',
...
new Ext.Button({
	...
	handler:function(){
			...
			if(pzlogGrid.getStore().getCount() > 0){
				
				if(pzlogGrid.getSelectionModel().getCount() >=1){
					if(confirm("确定要删除此照片信息吗")){
						selectPzrzid = pzlogGrid.getSelectionModel().getSelections()[0].data.pzrzid;
						var store = pzxxGrid.store;
        				store.baseParams = {
        						pzrzid:selectPzrzid,
         						config_key:'delPzxx',
         						start:0,
...
//					hidden:true,
					items:[{
						id: "panelHtmlId" ,
						html: '<div class="text" style="text-align:center;">拍照日志信息查询</div>',
						height: 80,
						region:'north',
	            ...
	            items:[
					{
		    		    ...
		    		    items:[new Ext.Button({
		        			...
		        			minWidth:60,
		        			handler:function(){
		        				var data = form20035.getForm().getValues();
								...
								if(p1_1.qybz){
									pzlogGrid.reconfigure(
											pzlogGrid.store,new Ext.grid.ColumnModel(Gnt.ZdyColumn(ary)));
									pzlogGrid.pxary = pxary;
								};
								pzlogGrid.zdyValueKey = Gnt.zdyValueKey(ary);
								var header=[];
								var shuxing = [];
								var zdyValueKey = Gnt.zdyValueKey(ary);
								pzlogGrid.type = 'pzlogcx';
		        				var store = pzlogGrid.store;
		        				store.removeAll();
		        				store.baseParams = data;
		        				var config=pzlogGrid.colModel.config;
								...
								store.baseParams.daochuFlag = 'pzlogcx';
	...
	var pzlogGrid = new Gnt.ux.SjpzGrid({
		...
		listeners:{
			rowclick:function(g, rowIndex, e){
				...
				curIndex = rowIndex;
				selectPzrzid = selRes.data.pzrzid; 			
			},
