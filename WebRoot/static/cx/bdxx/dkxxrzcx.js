...
var htmlStrStart = '<iframe scrolling="auto" frameborder="0" width="100%" height="100%" src="cx/bdcx/querydkxxrzcx';
...
Ext.onReady(function(){
	...
	//本业务需要加载的配置
	if(!Gnt.loadSjpzb("41001",function(){}))
		return;
	...
	var form41001 = new Gnt.ux.SjpzForm({
		pzlb: '41001',
		url: 'cx/bdcx/querydkxxrzcx',
		title:'',
		cols:2,
	...
	var NewForm41001 = new Gnt.ux.SjpzForm({
		closable: false,
		region:'center',
		/*height:500,*/
		pzlb: '41001',
...
//		bindStore:xblryGrid.store,
	...
	//        	height: 300,
			...
			},
			{
				...
				items:[{
						id: "panelHtmlId" ,
						html: '<div class="text" style="text-align:center;">读卡信息日志查询</div>',
						height: 80,
						region:'north',
	            ...
	            items:[
					{
		    		    ...
		    		    items:[new Ext.Button({
		        			...
		        			handler:function(){
		        				...
		        				var data = form41001.getForm().getValues();
	...
	var hxxGrid = new Gnt.ux.SjpzGrid({
		...
		listeners:{
			rowclick:function(g, rowIndex, e){
				...
				curIndex = rowIndex;
				selectdkrzid = selRes.data.dkrzid; 			
			},
			rowdblclick:function(grid,row){
				selRes = grid.store.getAt(row);
				selectdkrzid = selRes.data.dkrzid;
				NewForm41001.getForm().loadRecord(selRes);
				Gnt.toFormReadyonly(NewForm41001);
