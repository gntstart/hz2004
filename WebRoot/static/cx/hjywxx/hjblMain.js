...
Ext.onReady(function(){
	...
	//本业务需要加载的配置
	if(!Gnt.loadSjpzb("10019,20010",function(){}))
		return;
	...
	var form20010 = new Gnt.ux.SjpzForm({
		pzlb: '20010',
		url: 'yw/queryRyxx',
		title:'',
		cols:2,
...
//					hidden:true,
					items:[{
						id: "panelHtmlId" ,
						html: '<div class="text" style="text-align:center;"><h2>户籍补录信息查询</h2></div>',
						height: 80,
						region:'north',
	            ...
	            items:[
					{
		    		    ...
		    		    items:[new Ext.Button({
		        			...
		        			minWidth:60,
		        			handler:function(){
		        				var data = form20010.getForm().getValues();
								...
								store.baseParams.daochuFlag = 'hjblcx';
		        				applyParam(store);
//		        				Ext.apply(store.baseParams, rkfwRadio.items);
	...
	var xxGrid = new Gnt.ux.SjpzGrid({
		pkname: 'rynbid',
		pzlb: '20010',
		url:'cx/hjywxx/hjblcx/getHjblxx.json',
		region:'center',
		title: '',
		dcFlag:true,
		showPaging:true,
...
//		}
	...
	if(getQueryParam("jumpToHjbl")){
		form20010.getForm().findField("ryid").setValue(getQueryParam("ryid"));
		Ext.getCmp("queryId").handler();
	} 
