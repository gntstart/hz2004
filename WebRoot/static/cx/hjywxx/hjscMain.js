...
Ext.onReady(function(){
	...
	//本业务需要加载的配置
	if(!Gnt.loadSjpzb("10019,20011",function(){}))
		return;
	...
	var form20011 = new Gnt.ux.SjpzForm({
		pzlb: '20011',
		url: 'yw/queryRyxx',
		title:'',
		cols:2,
...
//					hidden:true,
					items:[{
						...
						frame: false,
						id: "panelHtmlId" ,
						html: '<div class="text" style="text-align:center;"><h2>户籍删除信息查询</h2></div>',
						height: 80,
						region:'north',
...
//			},
			{
		    	...
		    	items: [{
		    			...
		    			xtype:'button',
		        			...
		        			minWidth:60,
		        			handler:function(){
		        				var data = form20011.getForm().getValues();
								...
								if(p1_1.qybz){
									hjscGrid.reconfigure(
											hjscGrid.store,new Ext.grid.ColumnModel(Gnt.ZdyColumn(ary)));
									hjscGrid.pxary = pxary;
								};
								hjscGrid.zdyValueKey = Gnt.zdyValueKey(ary);
								var header=[];
								var shuxing = [];
								var zdyValueKey = Gnt.zdyValueKey(ary);
								hjscGrid.type = 'hjsccx';
		        				var store = hjscGrid.store;
		        				store.removeAll();
		        				store.baseParams = data;
		        				var config=hjscGrid.colModel.config;
								...
								store.baseParams.daochuFlag = 'hjsccx';
		        				applyParam(store);
		        				...
		        				store.on("load",function(store) {  
		        					if(store.data.length > 0){
		        						if(getQueryParam("jumpToHjsc")){
		        							hjscGrid.fireEvent("rowdblclick",hjscGrid,0);
		        						}
		        					}
		        				},hjscGrid);
		        			}
	...
	var hxxGrid = new Gnt.ux.SjpzGrid({
		...
		pzlb: '20001',
		title: '',
		url: 'cx/hjywxx/hbbgcx/getHbbgxx1.json',
		showPaging:false
	});	
	var hjscGrid = new Gnt.ux.SjpzGrid({
		pkname: 'rynbid',
		pzlb: '20011',
		url:'cx/hjywxx/hjsccx/getHjscxx.json',
		region:'center',
		height:540,
		title: '',
		dcFlag:true,
...
//						var storehxx = hxxGrid.store;
//						storehxx.baseParams = {
//								hhid:selectHhid1,
//								config_key:'queryPoHJXX_HXXB1',
//								start:0,
//								limit:20
//						};
//						storehxx.load();
//						storehxx.on("load",function(store) {
//							if(storehxx.data.length > 0){
//								selectHhnbid1 = storehxx.getAt(0).data.hhnbid;
//							}
//						},hxxGrid); 
						var pkvalue = selRes.data[hjscForm.bindStore.pkname];
						var re = hjscForm.bindStore.getById(pkvalue);
						if(re){
							hjscForm.getForm().loadRecord(re);
						}
					}
					Ext.Msg.hide();
				},rkGrid); 
		...
		items:[hjscGrid],
			buttons:[
				new Ext.Button({
					...
					minWidth:80,
					handler:function(){
						if(hjscGrid.getStore().getCount() > 0){
							if(hjscGrid.getSelectionModel().getCount() ==1){
								var store = hxxGrid.store;
		        				store.removeAll();
		        				store.baseParams = {
		        						hhnbid:hjscGrid.getSelectionModel().getSelections()[0].data.hhnbid,
		        						start:0,
		        						limit:20
		        				};
		        				store.load();
				...
				new Ext.Button({
					...
					handler:function(){
    					if(hjscGrid.getStore().getCount() > 0){
    						if(hjscGrid.getSelectionModel().getCount() ==1){
    							rynbid = hjscGrid.getSelectionModel().getSelections()[0].data.rynbid;
    							selectRyid = hjscGrid.getSelectionModel().getSelections()[0].data.ryid;
	...
	var hjscForm = new Gnt.ux.SjpzForm({
		closable: false,
		/*region:'north',
		height:500,*/
		pzlb: '20011',
		labelWidth : 120,
		cols:2,
//		bindStore:hjscGrid.store,
		//当修改表单的时候，能够POST到store，如果不指定那么采用bindViewGrid.store
		bindViewGrid:hjscGrid,
		//当bindViewGrid点击的时候，初始化form，有些只需要bindStore，比如迁出业务中的迁出登记没有grid
		changeDictCust:function(cmb,res){
...
//		}
	...
	if(getQueryParam("jumpToHjsc")){
		form20011.getForm().findField("ryid").setValue(getQueryParam("ryid"));
		Ext.getCmp("queryId").handler();
	}     
	/**
		按下回车键执行查询操作
    ...
    store.on('load',function(s,records){
    	...
    	s.each(function(r){
    		...
    		if(0 == r.get("ryzt")){
    			...
    			hjscGrid.getView().getRow(girdcount).style.backgroundColor="red";
    		}else{
    			hjscGrid.getView().getRow(girdcount).style.backgroundColor="green";
    		}
