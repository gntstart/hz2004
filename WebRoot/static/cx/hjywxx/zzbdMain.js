...
Ext.onReady(function(){
	...
	//本业务需要加载的配置
	if(!Gnt.loadSjpzb("10019,20007",function(){}))
		return;
	...
	var form20007 = new Gnt.ux.SjpzForm({
		pzlb: '20007',
		url: 'yw/queryRyxx',
		title:'',
		cols:2,
...
//					hidden:true,
					items:[{
						id: "panelHtmlId" ,
						html: '<div class="text" style="text-align:center;"><h2>住址变动信息查询</h2></div>',
						height: 80,
						region:'north',
	            ...
	            items:[
					{
		    		    ...
		    		    items:[new Ext.Button({
		        			...
		        			minWidth:60,
		        			handler:function(){
		        				var data = form20007.getForm().getValues();
								...
								store.baseParams.daochuFlag = 'zzbdcx';
		        				applyParam(store);
					...
					handler:function(){
						if(xxGrid.getStore().getCount() > 0){
							if(xxGrid.getSelectionModel().getCount() ==1){
		        				...
		        				store.removeAll();
		        				store.baseParams = {
		        						hh:xxGrid.getSelectionModel().getSelections()[0].data.hh,
		        						mlpnbid:xxGrid.getSelectionModel().getSelections()[0].data.mlpnbid,
		        						start:0,
		        						limit:20
		        				};
		        				store.load();
...
//						   	 		xtype:'panel',
//						   	 		id:"ckxxcx",
//						   	 		url:"cx/hjjbxx/ckxx?hhnbid=" + selectHhnbid +"&ryid="+selectRyid+ "&ai=2",
//						   	 		frame:false,
//						   	 		iconCls:'otherfile',
//							   		title: "人口基本信息查询",
//								    tabTip: "常口信息查询"
...
//						    tabs.setActiveTab("ckxxcx");
		    		    ...
		    		    items:[new Ext.Button({
		        			...
		        			minWidth:60,
		        			handler:function(){
		        				var url = basePath + "cx/hjjbxx/hxxcx?jumpToHxxcx="+"1"+"&hhnbid=" + selectHhnbid1;
		    					if(parent && parent.openWorkSpace){
		    						parent.openWorkSpace(url,  "现户信息查询", "xhxxcx" + id);
		    					}else{
		    		    ...
		    		    items:[new Ext.Button({
		        			...
		        			minWidth:60,
		        			handler:function(){
		        				var url = basePath + "cx/hjywxx/hcybdcx?jumpToHcybdcx="+"1"+"&hhnbid=" + selectHhnbid1;
		    					if(parent && parent.openWorkSpace){
		    						parent.openWorkSpace(url,  "户成员变动情况信息查询", "xhcybdqkxxcx" + id);
		    					}else{
		    						window.location.href = url;
		    					}
		        			}
		    		    ...
		    		    items:[new Ext.Button({
		        			...
		        			minWidth:60,
		        			handler:function(){
		        				var url = basePath + "cx/hjjbxx/hxxcx?jumpToHxxcx="+"1"+"&hhnbid=" + selectYhhnbid;
		    					if(parent && parent.openWorkSpace){
		    						parent.openWorkSpace(url,  "原户信息查询", "yhxxcx" + id);
		    					}else{
		    						window.location.href = url;
		    					}
		    		    ...
		    		    items:[new Ext.Button({
		        			...
		        			minWidth:60,
		        			handler:function(){
		        				var url = basePath + "cx/hjywxx/hcybdcx?jumpToHcybdcx="+"1"+"&hhnbid=" + selectYhhnbid;
		    					if(parent && parent.openWorkSpace){
		    						parent.openWorkSpace(url,  "户成员变动情况信息查询", "yhcybdqkxxcx" + id);
		    					}else{
		    						window.location.href = url;
		    					}
		        			}
...
//		}
	...
	var v = new Ext.Viewport({
 	...
 	if(getQueryParam("jumpToZzbd")){
 		form20007.getForm().findField("ryid").setValue(getQueryParam("ryid"));
		Ext.getCmp("queryId").handler();
	} 
	/**
		按下回车键执行查询操作
