...
Ext.onReady(function(){
	...
	//本业务需要加载的配置
	if(!Gnt.loadSjpzb("10019,20008",function(){}))
		return;
...
//					hidden:true,
					items:[{
						id: "panelHtmlId" ,
						html: '<div class="text" style="text-align:center;"><h2>变更更正信息查询</h2></div>',
						height: 80,
						region:'north',
	            ...
	            items:[
					{
		    		    ...
		    		    items:[new Ext.Button({
		        			...
		        			minWidth:60,
		        			handler:function(){
		        				var data = form20008.getForm().getValues();
								...
								store.baseParams.daochuFlag = 'bggzcx';
		        				applyParam(store);
//		        				Ext.apply(store.baseParams, rkfwRadio.items);
	...
	var xxGrid = new Gnt.ux.SjpzGrid({
		pkname: 'bggzid',
		pzlb: '20008',
		url:'cx/hjywxx/bggzcx/getBggzxx.json',
		region:'center',
		height:540,
		title: '',
		checkboxFlag : true,
...
//					Ext.Msg.hide();
//				},rkGrid); 
				xxForm.getForm().loadRecord(selRes);
				...
				new Ext.Button({
					...
					handler:function(){
						if(xxGrid.getStore().getCount() > 0){
							var records=xxGrid.getSelectionModel().getSelections();
							if(records.length>0){
								var array=[];
								var ryido = records[0].data.ryid;
								var bggzidTemp = '';
								for(var i=0;i<records.length;i++){
									if(records[i].data.ryid==ryido){
										bggzidTemp =bggzidTemp+ records[i].data.bggzid+',';
									}else{
										alert("必须选择同一人的的变更更正内容!");
										return;
									}
						        };
						        var o={};
						        o.bggzid=bggzidTemp.substr(0,bggzidTemp.length-1);
								o.directTable="bggzxmzm";
								array.push(o);
						        printfunction(0,array,hzywid);
						        if(hzywid){
		    		    ...
		    		    items:[new Ext.Button({
		        			...
		        			handler:function(){
	   							Gnt.ux.Dict.getKzcs('10002',function(pz, user){
	   								...
	   								url += "&ywcode=107&ryid=" +  selRes.data.ryid;
...
//		}
	...
	if(getQueryParam("jumpToBggz")){
		form20008.getForm().findField("ryid").setValue(getQueryParam("ryid"));
		Ext.getCmp("qyeryBtn").handler();
	} 
	//户政业务处理
	if(hzywid && hzywid!=""){
...
//			pch:pch,
			...
			callback:function(jo, jolist){
				...
				hzywjo = jo;
				if(hzywjo){
					form20008.getForm().setValues({gmsfhm:hzywjo.bsqrsfz});
					Ext.getCmp("qyeryBtn").handler();
				}
