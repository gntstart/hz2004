...
Ext.onReady(function(){
	...
	//本业务需要加载的配置
	if(!Gnt.loadSjpzb("10019,20009",function(){}))
		return;
	...
	var form20009 = new Gnt.ux.SjpzForm({
		pzlb: '20009',
		url: 'yw/queryRyxx',
		title:'',
		cols:2,
...
//					hidden:true,
					items:[{
						id: "panelHtmlId" ,
						html: '<div class="text" style="text-align:center;"><h2>户别变更信息查询</h2></div>',
						height: 80,
						region:'north',
	            ...
	            items:[
					{
		    		    ...
		    		    items:[new Ext.Button({
		        			...
		        			minWidth:60,
		        			handler:function(){
		        				var data = form20009.getForm().getValues();
								...
								store.baseParams.daochuFlag = 'hbbgcx';
		        				applyParam(store);
//		        				Ext.apply(store.baseParams, rkfwRadio.items);
	...
	var xxGrid = new Gnt.ux.SjpzGrid({
		pkname: 'rynbid',
		pzlb: '20009',
		url:'cx/hjywxx/hbbgcx/getHbbgxx.json',
		region:'center',
//		height:540,
		title: '',
		dcFlag:true,
		    		    ...
		    		    items:[new Ext.Button({
		        			...
		        			handler:function(){
	   							Gnt.ux.Dict.getKzcs('10002',function(pz, user){
	   								...
	   								url += "&ywcode=110&ryid=" +  selRes.data.ryid;
...
//		}
	...
	if(getQueryParam("jumpToHbbg")){
		form20009.getForm().findField("ryid").setValue(getQueryParam("ryid"));
		Ext.getCmp("queryId").handler();
	} 
	/**
		按下回车键执行查询操作
