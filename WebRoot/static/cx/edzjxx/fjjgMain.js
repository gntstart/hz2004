...
Ext.onReady(function(){
	...
	//本业务需要加载的配置
	if(!Gnt.loadSjpzb("10019,30033",function(){}))
		return;
...
//					hidden:true,
					items:[{
						id: "panelHtmlId" ,
						html: '<div class="text" style="text-align:center;">证件分检查询</div>',
						height: 80,
						region:'north',
			    	...
			    	items: [
						{
			    		    ...
			    		    items:[new Ext.Button({
			        			...
			        			handler:function(){
									...
									if(p1_1.qybz){
										fjjgGrid.reconfigure(
												fjjgGrid.store,new Ext.grid.ColumnModel(Gnt.ZdyColumn(ary)));
										fjjgGrid.pxary = pxary;
									};
									fjjgGrid.zdyValueKey = Gnt.zdyValueKey(ary);
									var header=[];
									var shuxing = [];
									var zdyValueKey = Gnt.zdyValueKey(ary);
									fjjgGrid.type = 'ffjgcx';
			        				var store = fjjgGrid.store;
			        				store.removeAll();
			        				store.baseParams = data;
			        				var config=fjjgGrid.colModel.config;
									...
									store.baseParams.daochuFlag = 'ffjgcx';
			        				applyParam(store);
//			        				Ext.apply(store.baseParams, rkfwRadio.items);
	...
	var fjjgGrid = new Gnt.ux.SjpzGrid({
		pkname: 'fjjgid',
		pzlb: '30033',
		url:'cx/edzjxx/fjjgcx/getFjjgxx.json',
		region:'center',
		height:400,
		title: '',
		dcFlag:true,
    ...
    store.on('load',function(s,records){
    	...
    	s.each(function(r){
    		...
    		if(0 == r.get("ryzt")){
    			...
    			fjjgGrid.getView().getRow(girdcount).style.backgroundColor="red";
    		}else{
    			fjjgGrid.getView().getRow(girdcount).style.backgroundColor="green";
    		}
