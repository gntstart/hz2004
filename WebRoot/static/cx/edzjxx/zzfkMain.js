...
Ext.onReady(function(){
	...
	//本业务需要加载的配置
	if(!Gnt.loadSjpzb("10019,30021",function(){}))
		return;
	...
	var form30021 = new Gnt.ux.SjpzForm({
		pzlb: '30021',
		url: 'yw/queryRyxx',
		title:'',
		cols:2,
...
//					hidden:true,
					items:[{
						id: "panelHtmlId" ,
						html: '<div class="text" style="text-align:center;">制证反馈信息查询</div>',
						height: 80,
						region:'north',
...
//			},
			    	...
			    	items: [
						{
			    		    ...
			    		    items:[new Ext.Button({
			        			...
			        			minWidth:60,
			        			handler:function(){
			        				var data = form30021.getForm().getValues();
									...
									if(p1_1.qybz){
										zzfkGrid.reconfigure(
												zzfkGrid.store,new Ext.grid.ColumnModel(Gnt.ZdyColumn(ary)));
										zzfkGrid.pxary = pxary;
									};
									zzfkGrid.zdyValueKey = Gnt.zdyValueKey(ary);
									var header=[];
									var shuxing = [];
									var zdyValueKey = Gnt.zdyValueKey(ary);
									zzfkGrid.type = 'zzfkcx';
			        				var store = zzfkGrid.store;
			        				store.removeAll();
			        				store.baseParams = data;
			        				var config=zzfkGrid.colModel.config;
									...
									store.baseParams.daochuFlag = 'zzfkcx';
			        				applyParam(store);
	...
	var zzfkGrid = new Gnt.ux.SjpzGrid({
		pkname: 'rynbid',
		pzlb: '30021',
		url:'cx/edzjxx/zzfkcx/getZzfkxx.json',
		region:'center',
		height:400,
		title: '',
		dcFlag:true,
    ...
    store.on('load',function(s,records){
    	...
    	s.each(function(r){
    		...
    		if(0 == r.get("ryzt")){
    			...
    			zzfkGrid.getView().getRow(girdcount).style.backgroundColor="red";
    		}else{
    			zzfkGrid.getView().getRow(girdcount).style.backgroundColor="green";
    		}
