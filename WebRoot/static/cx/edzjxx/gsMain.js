...
Ext.onReady(function(){
	...
	//本业务需要加载的配置
	if(!Gnt.loadSjpzb("10019,30019",function(){}))
		return;
	...
	var form30019 = new Gnt.ux.SjpzForm({
		pzlb: '30019',
		url: 'yw/queryRyxx',
		title:'',
		cols:2,
...
//					hidden:true,
					items:[{
						id: "panelHtmlId" ,
						html: '<div class="text" style="text-align:center;">挂失信息查询</div>',
						height: 80,
						region:'north',
...
//			},
			    	...
			    	items: [
						{
			    		    ...
			    		    items:[new Ext.Button({
			        			...
			        			minWidth:60,
			        			handler:function(){
			        				var data = form30019.getForm().getValues();
									...
									if(p1_1.qybz){
										gsGrid.reconfigure(
												gsGrid.store,new Ext.grid.ColumnModel(Gnt.ZdyColumn(ary)));
										gsGrid.pxary = pxary;
									};
									gsGrid.zdyValueKey = Gnt.zdyValueKey(ary);
									var header=[];
									var shuxing = [];
									var zdyValueKey = Gnt.zdyValueKey(ary);
									gsGrid.type = 'gscx';
			        				var store = gsGrid.store;
			        				store.removeAll();
			        				store.baseParams = data;
			        				var config=gsGrid.colModel.config;
									...
									store.baseParams.daochuFlag = 'gscx';
			        				applyParam(store);
	...
	var gsGrid = new Gnt.ux.SjpzGrid({
		pkname: 'gsslid',
		pzlb: '30019',
		url:'cx/edzjxx/gscx/getGsxx.json',
		region:'center',
		height:400,
		title: '',
		dcFlag:true,
		...
		listeners:{
			...
			rowdblclick:function(g, rowIndex, e){
				...
				var pkvalue = selRes.data[gsxxForm.bindStore.pkname];
				var re = gsxxForm.bindStore.getById(pkvalue);
				if(re){
					gsxxForm.getForm().loadRecord(re);
				}
				Gnt.toFormReadyonly(gsxxForm);
	...
	var gsxxForm = new Gnt.ux.SjpzForm({
		closable: false,
		/*region:'north',*/
		height:500,
		pzlb: '30019',
    ...
    store.on('load',function(s,records){
    	...
    	s.each(function(r){
    		...
    		if(0 == r.get("ryzt")){
    			...
    			gsGrid.getView().getRow(girdcount).style.backgroundColor="red";
    		}else{
    			gsGrid.getView().getRow(girdcount).style.backgroundColor="green";
    		}
