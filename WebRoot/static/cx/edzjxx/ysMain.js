...
Ext.onReady(function(){
	...
	//本业务需要加载的配置
	if(!Gnt.loadSjpzb("10019,30016",function(){}))
		return;
	...
	var form30016 = new Gnt.ux.SjpzForm({
		pzlb: '30016',
		url: 'yw/queryRyxx',
		title:'',
		cols:2,
...
//					hidden:true,
					items:[{
						id: "panelHtmlId" ,
						html: '<div class="text" style="text-align:center;">验收信息查询</div>',
						height: 80,
						region:'north',
...
//			},
			    	...
			    	items: [
						{
			    		    ...
			    		    items:[new Ext.Button({
			        			...
			        			minWidth:60,
			        			handler:function(){
			        				var data = form30016.getForm().getValues();
									...
									if(p1_1.qybz){
										ysGrid.reconfigure(
												ysGrid.store,new Ext.grid.ColumnModel(Gnt.ZdyColumn(ary)));
										ysGrid.pxary = pxary;
									};
									ysGrid.zdyValueKey = Gnt.zdyValueKey(ary);
									var header=[];
									var shuxing = [];
									var zdyValueKey = Gnt.zdyValueKey(ary);
									ysGrid.type = 'yscx';
			        				var store = ysGrid.store;
			        				store.removeAll();
			        				store.baseParams = data;
			        				var config=ysGrid.colModel.config;
									...
									store.baseParams.daochuFlag = 'yscx';
			        				applyParam(store);
	...
	var ysGrid = new Gnt.ux.SjpzGrid({
		pkname: 'ysslid',
		pzlb: '30016',
		url:'cx/edzjxx/yscx/getYsxx.json',
		region:'center',
		height:400,
		title: '',
		dcFlag:true,
		...
		listeners:{
			...
			rowdblclick:function(g, rowIndex, e){
				...
				var pkvalue = selRes.data[ysxxForm.bindStore.pkname];
				var re = ysxxForm.bindStore.getById(pkvalue);
				if(re){
					ysxxForm.getForm().loadRecord(re);
				}
				Gnt.toFormReadyonly(ysxxForm);
	...
	var ysxxForm = new Gnt.ux.SjpzForm({
		closable: false,
		/*region:'north',*/
		height:500,
		pzlb: '30016',
    ...
    store.on('load',function(s,records){
    	...
    	s.each(function(r){
    		...
    		if(0 == r.get("ryzt")){
    			...
    			ysGrid.getView().getRow(girdcount).style.backgroundColor="red";
    		}else{
    			ysGrid.getView().getRow(girdcount).style.backgroundColor="green";
    		}
