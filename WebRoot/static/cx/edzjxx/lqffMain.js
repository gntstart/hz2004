...
Ext.onReady(function(){
	...
	//本业务需要加载的配置
	if(!Gnt.loadSjpzb("10019,30018",function(){}))
		return;
	...
	var form30018 = new Gnt.ux.SjpzForm({
		pzlb: '30018',
		url: 'yw/queryRyxx',
		title:'',
		cols:2,
...
//					hidden:true,
					items:[{
						id: "panelHtmlId" ,
						html: '<div class="text" style="text-align:center;">领证信息查询</div>',
						height: 80,
						region:'north',
...
//		        	bodyStyle: 'padding:10px 0px 0px 0px ',
			    	...
			    	items: [
						{
			    		    ...
			    		    items:[new Ext.Button({
			        			...
			        			minWidth:60,
			        			handler:function(){
			        				var data = form30018.getForm().getValues();
									...
									if(p1_1.qybz){
										lqffGrid.reconfigure(
												lqffGrid.store,new Ext.grid.ColumnModel(Gnt.ZdyColumn(ary)));
										lqffGrid.pxary = pxary;
									};
									lqffGrid.zdyValueKey = Gnt.zdyValueKey(ary);
									var header=[];
									var shuxing = [];
									var zdyValueKey = Gnt.zdyValueKey(ary);
									lqffGrid.type = 'lqffcx';
			        				var store = lqffGrid.store;
			        				store.removeAll();
			        				store.baseParams = data;
			        				var config=lqffGrid.colModel.config;
									...
									store.baseParams.daochuFlag = 'lqffcx';
			        				applyParam(store);
	...
	var lqffGrid = new Gnt.ux.SjpzGrid({
		pkname: 'lqffid',
		pzlb: '30018',
		url:'cx/edzjxx/lqffcx/getLqffxx.json',
		region:'center',
		height:400,
		title: '',
		dcFlag:true,
		...
		listeners:{
			...
			rowdblclick:function(g, rowIndex, e){
				...
				var pkvalue = selRes.data[lqffForm.bindStore.pkname];
				var re = lqffForm.bindStore.getById(pkvalue);
				if(re){
					lqffForm.getForm().loadRecord(re);
				}
				Gnt.toFormReadyonly(lqffForm);
	...
	var lqffForm = new Gnt.ux.SjpzForm({
		closable: false,
		/*region:'north',*/
		height:500,
		pzlb: '30018',
    ...
    store.on('load',function(s,records){
    	...
    	s.each(function(r){
    		...
    		if(0 == r.get("ryzt")){
    			...
    			lqffGrid.getView().getRow(girdcount).style.backgroundColor="red";
    		}else{
    			lqffGrid.getView().getRow(girdcount).style.backgroundColor="green";
    		}
