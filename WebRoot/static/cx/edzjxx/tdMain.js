...
Ext.onReady(function(){
	...
	//本业务需要加载的配置
	if(!Gnt.loadSjpzb("10019,30020",function(){}))
		return;
	...
	var form30020 = new Gnt.ux.SjpzForm({
		pzlb: '30020',
		url: 'yw/queryRyxx',
		title:'',
		cols:2,
...
//					hidden:true,
					items:[{
						id: "panelHtmlId" ,
						html: '<div class="text" style="text-align:center;">投递信息查询</div>',
						height: 80,
						region:'north',
...
//			},
			    	...
			    	items: [
						{
			    		    ...
			    		    items:[new Ext.Button({
			        			...
			        			minWidth:60,
			        			handler:function(){
			        				var data = form30020.getForm().getValues();
									...
									if(p1_1.qybz){
										tdGrid.reconfigure(
												tdGrid.store,new Ext.grid.ColumnModel(Gnt.ZdyColumn(ary)));
										tdGrid.pxary = pxary;
									};
									tdGrid.zdyValueKey = Gnt.zdyValueKey(ary);
									var header=[];
									var shuxing = [];
									var zdyValueKey = Gnt.zdyValueKey(ary);
									tdGrid.type = 'tdcx';
			        				var store = tdGrid.store;
			        				store.removeAll();
			        				store.baseParams = data;
			        				var config=tdGrid.colModel.config;
									...
									store.baseParams.daochuFlag = 'tdcx';
			        				applyParam(store);
	...
	var tdGrid = new Gnt.ux.SjpzGrid({
		pkname: 'nbslid',
		pzlb: '30020',
		url:'cx/edzjxx/tdcx/getTdxx.json',
		region:'center',
		height:400,
		title: '',
		dcFlag:true,
    ...
    store.on('load',function(s,records){
    	...
    	s.each(function(r){
    		...
    		if(0 == r.get("ryzt")){
    			...
    			tdGrid.getView().getRow(girdcount).style.backgroundColor="red";
    		}else{
    			tdGrid.getView().getRow(girdcount).style.backgroundColor="green";
    		}
