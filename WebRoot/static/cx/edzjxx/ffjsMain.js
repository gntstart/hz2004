...
Ext.onReady(function(){
	...
	//本业务需要加载的配置
	if(!Gnt.loadSjpzb("10019,30034",function(){}))
		return;
	...
	var form30034 = new Gnt.ux.SjpzForm({
		pzlb: '30034',
		url: 'yw/queryRyxx',
		title:'',
		cols:2,
...
//					hidden:true,
					items:[{
						id: "panelHtmlId" ,
						html: '<div class="text" style="text-align:center;">证件分发接收查询</div>',
						height: 80,
						region:'north',
...
//		        	bodyStyle: 'padding:10px 0px 0px 0px ',
			    	...
			    	items: [
						{
			    		    ...
			    		    items:[new Ext.Button({
			        			...
			        			minWidth:60,
			        			handler:function(){
			        				var data = form30034.getForm().getValues();
									...
									if(p1_1.qybz){
										ffjsGrid.reconfigure(
												ffjsGrid.store,new Ext.grid.ColumnModel(Gnt.ZdyColumn(ary)));
										ffjsGrid.pxary = pxary;
									};
									ffjsGrid.zdyValueKey = Gnt.zdyValueKey(ary);
									var header=[];
									var shuxing = [];
									var zdyValueKey = Gnt.zdyValueKey(ary);
									ffjsGrid.type = 'ffjscx';
			        				var store = ffjsGrid.store;
			        				store.removeAll();
			        				store.baseParams = data;
			        				var config=ffjsGrid.colModel.config;
									...
									store.baseParams.daochuFlag = 'ffjscx';
			        				applyParam(store);
	...
	var ffjsGrid = new Gnt.ux.SjpzGrid({
		pkname: 'ffjsid',
		pzlb: '30034',
		url:'cx/edzjxx/ffjscx/getFfjsxx.json',
		region:'center',
		height:400,
		title: '',
		dcFlag:true,
    ...
    store.on('load',function(s,records){
    	...
    	s.each(function(r){
    		...
    		if(0 == r.get("ryzt")){
    			...
    			ffjsGrid.getView().getRow(girdcount).style.backgroundColor="red";
    		}else{
    			ffjsGrid.getView().getRow(girdcount).style.backgroundColor="green";
    		}
