...
Ext.onReady(function(){
	...
	//本业务需要加载的配置
	if(!Gnt.loadSjpzb("10019,20032,20024,10031",function(){}))
		return;
	...
	var form20032 = new Gnt.ux.SjpzForm({
		pzlb: '20032',
		url: 'yw/queryRyxx',
		title:'',
		cols:2,
...
//					hidden:true,
					items:[{
						...
						frame: false,
							id: "panelHtmlId" ,
							html: '<div class="text" style="text-align:center;"><br /><h2>户别变更审批</ h2></div>',
							height: 80,
							region:'north',
...
//			},
        	   ...
        	   layoutConfig: {
		    	...
		    	items: [{
	    			...
	    			xtype:'button',
	        			...
	        			handler:function(){
							//记录日志的操作码
	        				var data = form20032.getForm().getValues();
							...
							if(p1_1.qybz){
								hbbgspGrid.reconfigure(
										hbbgspGrid.store,new Ext.grid.ColumnModel(Gnt.ZdyColumn(ary)));
								hbbgspGrid.pxary = pxary;
							};
							hbbgspGrid.zdyValueKey = Gnt.zdyValueKey(ary);
							var header=[];
							var shuxing = [];
							var zdyValueKey = Gnt.zdyValueKey(ary);
							hbbgspGrid.type = 'hbbgspcx';
	        				var store = hbbgspGrid.store;
	        				store.removeAll();
	        				store.baseParams = data;
	        				var config=hbbgspGrid.colModel.config;
							...
							store.baseParams.daochuFlag = 'hbbgspcx';
	        				applyParam(store);
	        				...
	        				store.on('load',function(s,records){
	        					if(store.getCount() > 0){
		        					...
		        					hbbgspGrid.fireEvent("rowclick",hbbgspGrid,0);
		        					hbbgspGrid.getSelectionModel().selectRange(0,0);
		        					if(hzywid != null){
		        						hbbgspGrid.fireEvent("rowdblclick",hbbgspGrid,0);
		        					}
	        					}else{
	        						showInfo("未查到有关信息！");
	        					}
	...
	var hbbgspGrid = new Gnt.ux.SjpzGrid({
		pkname: 'rynbid',
		pzlb: '20032',
		url:'cx/spcx/hbbgspcx/getHbbgsp.json',
		region:'center',
		height:540,
		title: '户别变更审批信息',
		dcFlag:true,
	...
	var hbbgspzbGrid = new Gnt.ux.SjpzGrid({
		pkname: 'rynbid',
		pzlb: '10031',
	...
	if(hzywid && hzywid!=""){
		Gnt.makeHzyw({
			...
			callback:function(jo, jolist){
				...
				form20032.getForm().findField("gmsfhm").setValue(jo.bsqrsfz);
    ...
    store.on('load',function(s,records){
    	...
    	s.each(function(r){
    		...
    		if(0 == r.get("ryzt")){
    			...
    			hbbgspGrid.getView().getRow(girdcount).style.backgroundColor="red";
    		}else{
    			hbbgspGrid.getView().getRow(girdcount).style.backgroundColor="green";
    		}
