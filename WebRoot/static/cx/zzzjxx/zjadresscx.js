...
var htmlStrStart = '<iframe scrolling="auto" frameborder="0" width="100%" height="100%" src="cx/zzzj/queryzjAdresscx';
...
Ext.onReady(function(){
	...
	//本业务需要加载的配置
	if(!Gnt.loadSjpzb("30030",function(){}))
		return;
	...
	var form30030 = new Gnt.ux.SjpzForm({
		pzlb: '30030',
		url: 'cx/zzzj/queryzjAdresscx',
		title:'',
		cols:2,
...
//	var zjzzGrid = new Gnt.ux.SjpzGrid({
//		pkname: 'zzzjid',
//		pzlb: '30030',
//		title: '',
//		url: 'cx/zzzj/queryzjAdresscx',
...
//	});
	...
	var NewForm30030 = new Gnt.ux.SjpzForm({
		closable: false,
		region:'center',
		/*height:500,*/
		pzlb: '30030',
...
//					hidden:true,
					items:[{
						id: "panelHtmlId" ,
						html: '<div class="text" style="text-align:center;">追加住址查询</div>',
						height: 80,
						region:'north',
	            ...
	            items:[
					{
		    		    ...
		    		    items:[new Ext.Button({
		        			...
		        			minWidth:60,
		        			handler:function(){
		        				var data = form30030.getForm().getValues();
								...
								if(p1_1.qybz){
									zjzzGrid.reconfigure(
											zjzzGrid.store,new Ext.grid.ColumnModel(Gnt.ZdyColumn(ary)));
									zjzzGrid.pxary = pxary;
								};
								zjzzGrid.zdyValueKey = Gnt.zdyValueKey(ary);
								var header=[];
								var shuxing = [];
								var zdyValueKey = Gnt.zdyValueKey(ary);
								zjzzGrid.type = 'zjadresscx';
		        				var store = zjzzGrid.store;
		        				store.removeAll();
		        				store.baseParams = data;
		        				var config=zjzzGrid.colModel.config;
								...
								store.baseParams.daochuFlag = 'zjadresscx';
	...
	var zjzzGrid = new Gnt.ux.SjpzGrid({
		...
		listeners:{
			rowclick:function(g, rowIndex, e){
				...
				curIndex = rowIndex;
				selectzzzjid = selRes.data.zzzjid; 			
			},
			rowdblclick:function(grid,row){
				selRes = grid.store.getAt(row);
				selectzzzjid = selRes.data.zzzjid;
				NewForm30030.getForm().loadRecord(selRes);
				Gnt.toFormReadyonly(NewForm30030);
