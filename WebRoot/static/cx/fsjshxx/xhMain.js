...
Ext.onReady(function(){
	...
	//本业务需要加载的配置
	if(!Gnt.loadSjpzb("10019,30023",function(){}))
		return;
	...
	var form30023 = new Gnt.ux.SjpzForm({
		pzlb: '30023',
		url: 'yw/queryRyxx',
		title:'',
		cols:2,
...
//					hidden:true,
					items:[{
						id: "panelHtmlId" ,
						html: '<div class="text" style="text-align:center;">销毁信息查询</div>',
						height: 80,
						region:'north',
...
//			},
	            ...
	            items:[
					{
		    		    ...
		    		    items:[new Ext.Button({
		        			...
		        			handler:function(){
		        				...
		        				var data = form30023.getForm().getValues();
								...
								if(p1_1.qybz){
									xhGrid.reconfigure(
											xhGrid.store,new Ext.grid.ColumnModel(Gnt.ZdyColumn(ary)));
									xhGrid.pxary = pxary;
								};
								xhGrid.zdyValueKey = Gnt.zdyValueKey(ary);
								var header=[];
								var shuxing = [];
								var zdyValueKey = Gnt.zdyValueKey(ary);
								xhGrid.type = 'xhcx';
								var store = xhGrid.store;
		        				store.removeAll();
		        				store.baseParams = data;
		        				var config=xhGrid.colModel.config;
								...
								store.baseParams.daochuFlag = 'xhcx';
		        				applyParam(store);
	...
	var xhGrid = new Gnt.ux.SjpzGrid({
		pkname: 'xhxxid',
		pzlb: '30023',
		url:'cx/fsjshxx/xhcx/getXhxx.json',
		region:'center',
		height:400,
		title: '',
		dcFlag:true,
    ...
    store.on('load',function(s,records){
    	...
    	s.each(function(r){
    		...
    		if(0 == r.get("ryzt")){
    			...
    			xhGrid.getView().getRow(girdcount).style.backgroundColor="red";
    		}else{
    			xhGrid.getView().getRow(girdcount).style.backgroundColor="green";
    		}
