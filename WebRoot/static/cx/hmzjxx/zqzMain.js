...
Ext.onReady(function(){
	...
	//本业务需要加载的配置
	if(!Gnt.loadSjpzb("10019,20029",function(){}))
		return;
	...
	var form20029 = new Gnt.ux.SjpzForm({
		pzlb: '20029',
		url: 'yw/queryRyxx',
		title:'',
		cols:2,
...
//					hidden:true,
					items:[{
						id: "panelHtmlId" ,
						html: '<div class="text" style="text-align:center;">准迁证查询</div>',
						height: 80,
						region:'north',
...
//			},
			{
	            ...
	            items:[
				{
	    		    ...
	    		    items:[new Ext.Button({
	        			...
	        			minWidth:60,
	        			handler:function(){
	        				var data = form20029.getForm().getValues();
							...
							if(p1_1.qybz){
								zqzGrid.reconfigure(
										zqzGrid.store,new Ext.grid.ColumnModel(Gnt.ZdyColumn(ary)));
								zqzGrid.pxary = pxary;
							};
							zqzGrid.zdyValueKey = Gnt.zdyValueKey(ary);
							var header=[];
							var shuxing = [];
							var zdyValueKey = Gnt.zdyValueKey(ary);
							zqzGrid.type = 'zqzcx';
	        				var store = zqzGrid.store;
	        				store.removeAll();
	        				store.baseParams = data;
	        				var config=zqzGrid.colModel.config;
							...
							store.baseParams.daochuFlag = 'zqzcx';
	        				applyParam(store);
	...
	var zqzGrid = new Gnt.ux.SjpzGrid({
		pkname: 'zqid',
		pzlb: '20029',
		url:'cx/hmzjxx/zqzcx/getZqzxx.json',
		region:'center',
		height:400,
		title: '',
		dcFlag:true,
    ...
    store.on('load',function(s,records){
    	...
    	s.each(function(r){
    		...
    		if(0 == r.get("ryzt")){
    			...
    			zqzGrid.getView().getRow(girdcount).style.backgroundColor="red";
    		}else{
    			zqzGrid.getView().getRow(girdcount).style.backgroundColor="green";
    		}
