...
Ext.onReady(function(){
	...
	//本业务需要加载的配置
	if(!Gnt.loadSjpzb("10019,20028",function(){}))
		return;
	...
	var form20028 = new Gnt.ux.SjpzForm({
		pzlb: '20028',
		url: 'yw/queryRyxx',
		title:'',
		cols:2,
...
//					hidden:true,
					items:[{
						id: "panelHtmlId" ,
						html: '<div class="text" style="text-align:center;">迁移证查询</div>',
						height: 80,
						region:'north',
...
//			},
		    	...
		    	items: [
					{
		    		    ...
		    		    items:[new Ext.Button({
		        			...
		        			minWidth:60,
		        			handler:function(){
		        				var data = form20028.getForm().getValues();
								...
								if(p1_1.qybz){
									qyzGrid.reconfigure(
											qyzGrid.store,new Ext.grid.ColumnModel(Gnt.ZdyColumn(ary)));
									qyzGrid.pxary = pxary;
								};
								qyzGrid.zdyValueKey = Gnt.zdyValueKey(ary);
								var header=[];
								var shuxing = [];
								var zdyValueKey = Gnt.zdyValueKey(ary);
								qyzGrid.type = 'qyzcx';
		        				var store = qyzGrid.store;
		        				store.removeAll();
		        				store.baseParams = data;
		        				var config=qyzGrid.colModel.config;
								...
								store.baseParams.daochuFlag = 'qyzcx';
		        				applyParam(store);
	...
	var qyzGrid = new Gnt.ux.SjpzGrid({
		pkname: 'qyid',
		pzlb: '20028',
		url:'cx/hmzjxx/qyzcx/getQyzxx.json',
		region:'center',
		height:400,
		title: '',
		dcFlag:true,
    ...
    store.on('load',function(s,records){
    	...
    	s.each(function(r){
    		...
    		if(0 == r.get("ryzt")){
    			...
    			qyzGrid.getView().getRow(girdcount).style.backgroundColor="red";
    		}else{
    			qyzGrid.getView().getRow(girdcount).style.backgroundColor="green";
    		}
