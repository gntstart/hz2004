...
Ext.onReady(function(){
	...
	//本业务需要加载的配置
	if(!Gnt.loadSjpzb("10019,30013",function(){}))
		return;
	...
	var form30013 = new Gnt.ux.SjpzForm({
		pzlb: '30013',
		url: 'yw/queryRyxx',
		title:'',
		cols:2,
...
//					hidden:true,
					items:[{
						id: "panelHtmlId" ,
						html: '<div class="text" style="text-align:center;">身份号分配信息查询</div>',
						height: 80,
						region:'north',
...
//			},
		    	...
		    	items: [
					{
		    		    ...
		    		    items:[new Ext.Button({
		        			...
		        			minWidth:60,
		        			handler:function(){
		        				var data = form30013.getForm().getValues();
								...
								if(p1_1.qybz){
									sfhfpGrid.reconfigure(
											sfhfpGrid.store,new Ext.grid.ColumnModel(Gnt.ZdyColumn(ary)));
									sfhfpGrid.pxary = pxary;
								};
								sfhfpGrid.zdyValueKey = Gnt.zdyValueKey(ary);
								var header=[];
								var shuxing = [];
								var zdyValueKey = Gnt.zdyValueKey(ary);
								sfhfpGrid.type = 'sfhfpcx';
		        				var store = sfhfpGrid.store;
		        				store.removeAll();
		        				store.baseParams = data;
		        				var config=sfhfpGrid.colModel.config;
								...
								store.baseParams.daochuFlag = 'sfhfpcx';
		        				applyParam(store);
	...
	var sfhfpGrid = new Gnt.ux.SjpzGrid({
		pkname: 'fpid',
		pzlb: '30013',
		url:'cx/hmzjxx/sfhfpcx/getSfhfpxx.json',
		region:'center',
		height:400,
		title: '',
		dcFlag:true,
    ...
    store.on('load',function(s,records){
    	...
    	s.each(function(r){
    		...
    		if(0 == r.get("ryzt")){
    			...
    			sfhfpGrid.getView().getRow(girdcount).style.backgroundColor="red";
    		}else{
    			sfhfpGrid.getView().getRow(girdcount).style.backgroundColor="green";
    		}
