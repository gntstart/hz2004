...
Ext.onReady(function(){
	...
	//本业务需要加载的配置
	if(!Gnt.loadSjpzb("10019,30012",function(){}))
		return;
	...
	var form30012 = new Gnt.ux.SjpzForm({
		pzlb: '30012',
		url: 'yw/queryRyxx',
		title:'',
		cols:2,
...
//					hidden:true,
					items:[{
						id: "panelHtmlId" ,
						html: '<div class="text" style="text-align:center;">重号反馈信息查询</div>',
						height: 80,
						region:'north',
...
//			},
		    	...
		    	items: [
					{
		    		    ...
		    		    items:[new Ext.Button({
		        			...
		        			minWidth:60,
		        			handler:function(){
		        				var data = form30012.getForm().getValues();
								...
								if(p1_1.qybz){
									chGrid.reconfigure(
											chGrid.store,new Ext.grid.ColumnModel(Gnt.ZdyColumn(ary)));
									chGrid.pxary = pxary;
								};
								chGrid.zdyValueKey = Gnt.zdyValueKey(ary);
								var header=[];
								var shuxing = [];
								var zdyValueKey = Gnt.zdyValueKey(ary);
								chGrid.type = 'chcx';
		        				var store = chGrid.store;
		        				store.removeAll();
		        				store.baseParams = data;
		        				var config=chGrid.colModel.config;
								...
								store.baseParams.daochuFlag = 'chcx';
		        				applyParam(store);
	...
	var chGrid = new Gnt.ux.SjpzGrid({
		pkname: 'chid',
		pzlb: '30012',
		url:'cx/hmzjxx/chcx/getChxx.json',
		region:'center',
		height:400,
		title: '',
		dcFlag:true,
    ...
    store.on('load',function(s,records){
    	...
    	s.each(function(r){
    		...
    		if(0 == r.get("ryzt")){
    			...
    			chGrid.getView().getRow(girdcount).style.backgroundColor="red";
    		}else{
    			chGrid.getView().getRow(girdcount).style.backgroundColor="green";
    		}
