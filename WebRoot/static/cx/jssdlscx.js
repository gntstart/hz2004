...
Ext.onReady(function(){
	...
	//本业务需要加载的配置
	if(!Gnt.loadSjpzb("10019,50003",function(){}))
		return;
	...
	var form50003 = new Gnt.ux.SjpzForm({
		pzlb: '50003',
		url: 'yw/queryRyxx',
		title:'',
		cols:2,
...
//					hidden:true,
					items:[{
						id: "panelHtmlId" ,
						html: '<div class="text" style="text-align:center;">解锁锁定历史查询</div>',
						height: 80,
						region:'north',
			    	...
			    	items: [
						{
			    		    ...
			    		    items:[new Ext.Button({
			        			...
			        			minWidth:60,
			        			handler:function(){
			        				var data = form50003.getForm().getValues();
									...
									if(p1_1.qybz){
										jssdGrid.reconfigure(
												jssdGrid.store,new Ext.grid.ColumnModel(Gnt.ZdyColumn(ary)));
										jssdGrid.pxary = pxary;
									};
									jssdGrid.zdyValueKey = Gnt.zdyValueKey(ary);
									var header=[];
									var shuxing = [];
									var zdyValueKey = Gnt.zdyValueKey(ary);
									jssdGrid.type = 'queryJssdls';
			        				var store = jssdGrid.store;
			        				store.removeAll();
			        				store.baseParams = data;
			        				var config=jssdGrid.colModel.config;
									...
									store.baseParams.daochuFlag = 'queryJssdls';
			        				applyParam(store);
//			        				Ext.apply(store.baseParams, rkfwRadio.items);
	...
	var jssdGrid = new Gnt.ux.SjpzGrid({
		pkname: 'jssdlogxxbid',
		pzlb: '50003',
		url:'cx/jssdlscx/queryJssdls.json',
		region:'center',
		height:400,
		title: '',
		dcFlag:true,
		...
		listeners:{
			...
			rowdblclick:function(g, rowIndex, e){
				...
				var pkvalue = selRes.data[jssdForm.bindStore.pkname];
    ...
    store.on('load',function(s,records){
    	...
    	s.each(function(r){
    		...
    		if(0 == r.get("ryzt")){
    			...
    			jssdGrid.getView().getRow(girdcount).style.backgroundColor="red";
    		}else{
    			jssdGrid.getView().getRow(girdcount).style.backgroundColor="green";
    		}
