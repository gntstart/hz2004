...
Ext.onReady(function(){
	...
	var bczryxxGrid = new Gnt.ux.SjpzGrid({
		pkname: 'rynbid',
		region:'center',
        height:430,
		pzlb: '20000',
...
//		bindStore:xblryGrid.store,
		...
		//items:[bczryxxGrid,form20008,{
			items:[{region:'north',
		        ...
		        buttons:[ new Ext.Button({
		        	...
		        	minWidth:100,
		        	handler:function(){
		        		var data=bczryxxGrid.store.data.items;
		        		sjcx_window.show();
		        		sjcx_window.setSelRes(data,"bczryxx");
		        	}
		        })]
		        },bczryxxGrid,{
			region:'south',
	...
	store.on("load",function(store) {  
		if(store.data.length > 0){
			curIndex = 0;
			bczryxxGrid.fireEvent("rowclick",bczryxxGrid,curIndex);
			bczryxxGrid.getSelectionModel().selectRange(curIndex,curIndex);
		}
