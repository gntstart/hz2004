...
Ext.onReady(function(){
	...
	var czryxxGrid = new Gnt.ux.SjpzGrid({
		pkname: 'hhnbid',
		region:'center',
        height:430,
		pzlb: '20000',
...
//		bindStore:xblryGrid.store,
		...
		//items:[czryxxGrid,form20008,{
			items:[{region:'north',
		        ...
		        buttons:[ new Ext.Button({
		        	...
		        	minWidth:100,
		        	handler:function(){
		        		var data=czryxxGrid.store.data.items;
		        		sjcx_window.show();
		        		sjcx_window.setSelRes(data,"czryxx");
		        	}
		        })]
		        },czryxxGrid,{
			region:'south',
	...
	store.on("load",function(store) {  
		if(store.data.length > 0){
			curIndex = 0;
			czryxxGrid.fireEvent("rowclick",czryxxGrid,curIndex);
			czryxxGrid.getSelectionModel().selectRange(curIndex,curIndex);
		}
