...
Ext.onReady(function(){
	...
	var bczhxxGrid = new Gnt.ux.SjpzGrid({
		pkname: 'mlpnbid',
		region:'center',
        height:430,
		pzlb: '20001',
...
//		bindStore:xblryGrid.store,
		...
		//items:[bczhxxGrid,form20008,{
			items:[{region:'north',
		        ...
		        buttons:[ new Ext.Button({
		        	...
		        	minWidth:100,
		        	handler:function(){
		        		var data=bczhxxGrid.store.data.items;
		        		sjcx_window.show();
		        		sjcx_window.setSelRes(data,"bczhxx");
		        	}
		        })]
		        },bczhxxGrid,{
			region:'south',
	...
	store.on("load",function(store) {  
		if(store.data.length > 0){
			curIndex = 0;
			bczhxxGrid.fireEvent("rowclick",bczhxxGrid,curIndex);
			bczhxxGrid.getSelectionModel().selectRange(curIndex,curIndex);
		}
