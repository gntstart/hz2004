...
Ext.onReady(function(){
	...
	var qszpGrid = new Gnt.ux.SjpzGrid({
		pkname: 'rynbid',
		region:'center',
        height:430,
		pzlb: '20000',
...
//		bindStore:xblryGrid.store,
		...
		//items:[qszpGrid,form20008,{
			items:[{region:'north',
		        ...
		        buttons:[ new Ext.Button({
		        	...
		        	minWidth:100,
		        	handler:function(){
		        		var data=qszpGrid.store.data.items;
		        		sjcx_window.show();
		        		sjcx_window.setSelRes(data,"qszp");
		        	}
		        })]
		        },qszpGrid,{
			region:'south',
	...
	store.on("load",function(store) {  
		if(store.data.length > 0){
			curIndex = 0;
			qszpGrid.fireEvent("rowclick",qszpGrid,curIndex);
			qszpGrid.getSelectionModel().selectRange(curIndex,curIndex);
		}
