...
Ext.onReady(function(){
	...
	var ChcxGrid = new Gnt.ux.SjpzGrid({
		pkname: 'rynbid',
		region:'center',
        height:430,
		pzlb: '20000',
...
//		bindStore:xblryGrid.store,
		...
		//items:[ChcxGrid,form20008,{
			items:[{region:'north',
		        ...
		        buttons:[ new Ext.Button({
		        	...
		        	minWidth:100,
		        	handler:function(){
		        		var data=ChcxGrid.store.data.items;
		        		sjcx_window.show();
		        		sjcx_window.setSelRes(data,"chcx");
		        	}
		        })]
		        },ChcxGrid,{
			region:'south',
	...
	store.on("load",function(store) {  
		if(store.data.length > 0){
			curIndex = 0;
			ChcxGrid.fireEvent("rowclick",ChcxGrid,curIndex);
			ChcxGrid.getSelectionModel().selectRange(curIndex,curIndex);
		}
