...
Ext.onReady(function(){
	...
	var yhdghzGrid = new Gnt.ux.SjpzGrid({
		pkname: 'rynbid',
		region:'center',
        height:430,
		pzlb: '20000',
...
//		bindStore:xblryGrid.store,
		...
		//items:[yhdghzGrid,form20008,{
			items:[{region:'north',
		        ...
		        buttons:[ new Ext.Button({
		        	...
		        	minWidth:100,
		        	handler:function(){
		        		var data=yhdghzGrid.store.data.items;
		        		sjcx_window.show();
		        		sjcx_window.setSelRes(data,"yhdghz");
		        	}
		        })]
		        },yhdghzGrid,{
			region:'south',
	...
	store.on("load",function(store) {  
		if(store.data.length > 0){
			curIndex = 0;
			yhdghzGrid.fireEvent("rowclick",yhdghzGrid,curIndex);
			yhdghzGrid.getSelectionModel().selectRange(curIndex,curIndex);
		}
