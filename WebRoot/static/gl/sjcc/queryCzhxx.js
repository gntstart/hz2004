...
Ext.onReady(function(){
	...
	var czhxxGrid = new Gnt.ux.SjpzGrid({
		pkname: 'hhnbid',
		region:'center',
        height:430,
		pzlb: '20001',
...
//		bindStore:xblryGrid.store,
		...
		//items:[czhxxGrid,form20008,{
			items:[{region:'north',
		        ...
		        buttons:[ new Ext.Button({
		        	...
		        	minWidth:100,
		        	handler:function(){
		        		var data=czhxxGrid.store.data.items;
		        		sjcx_window.show();
		        		sjcx_window.setSelRes(data,"czhxx");
		        	}
		        })]
		        },czhxxGrid,{
			region:'south',
	...
	store.on("load",function(store) {  
		if(store.data.length > 0){
			curIndex = 0;
			czhxxGrid.fireEvent("rowclick",czhxxGrid,curIndex);
			czhxxGrid.getSelectionModel().selectRange(curIndex,curIndex);
		}
