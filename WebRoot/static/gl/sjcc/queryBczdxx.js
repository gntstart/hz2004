...
Ext.onReady(function(){
	...
	var bczdxxGrid = new Gnt.ux.SjpzGrid({
		pkname: 'mlpnbid',
		region:'center',
        height:430,
		pzlb: '20002',
...
//		bindStore:xblryGrid.store,
		...
		//items:[bczdxxGrid,form20008,{
			items:[{region:'north',
		        ...
		        buttons:[ new Ext.Button({
		        	...
		        	minWidth:100,
		        	handler:function(){
		        		var data=bczdxxGrid.store.data.items;
		        		sjcx_window.show();
		        		sjcx_window.setSelRes(data,"bczdxx");
		        	}
		        })]
		        },bczdxxGrid,{
			region:'south',
	...
	store.on("load",function(store) {  
		if(store.data.length > 0){
			curIndex = 0;
			bczdxxGrid.fireEvent("rowclick",bczdxxGrid,curIndex);
			bczdxxGrid.getSelectionModel().selectRange(curIndex,curIndex);
		}
