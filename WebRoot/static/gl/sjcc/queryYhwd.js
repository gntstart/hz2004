...
Ext.onReady(function(){
	...
	var yhwdGrid = new Gnt.ux.SjpzGrid({
		pkname: 'hhnbid',
		region:'center',
        height:430,
		pzlb: '20020',
...
//		bindStore:xblryGrid.store,
		...
		//items:[yhwdGrid,form20008,{
			items:[{region:'north',
		        ...
		        buttons:[ new Ext.Button({
		        	...
		        	minWidth:100,
		        	handler:function(){
		        		var data=yhwdGrid.store.data.items;
		        		sjcx_window.show();
		        		sjcx_window.setSelRes(data,"yhwd");
		        	}
		        })]
		        },yhwdGrid,{
			region:'south',
	...
	store.on("load",function(store) {  
		if(store.data.length > 0){
			curIndex = 0;
			yhwdGrid.fireEvent("rowclick",yhwdGrid,curIndex);
			yhwdGrid.getSelectionModel().selectRange(curIndex,curIndex);
		}
