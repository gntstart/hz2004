...
Ext.onReady(function(){
	...
	var bgdykzGrid = new Ext.grid.GridPanel({
        store: myuserStore,
        region: 'center',
									...
									for(var i= 0;i<store.data.length;i++){
										...
										if(Ext.getCmp('queryFlag').getValue()==1){//字段名称
											...
											if(a.indexOf(b.toUpperCase())!=-1){
												bgdykzGrid.fireEvent("rowclick",bgdykzGrid,i);
												bgdykzGrid.getSelectionModel().selectRange(i,i);
												bgdykzGrid.getView().focusRow(i);
												return;
											}
...
//	};
	...
	store.on("load",function(store) {  
		if(store.data.length > 0){
			curIndex = 0;
			bgdykzGrid.fireEvent("rowclick",bgdykzGrid,curIndex);
			bgdykzGrid.getSelectionModel().selectRange(curIndex,curIndex);
		}
	},bgdykzGrid); 
});